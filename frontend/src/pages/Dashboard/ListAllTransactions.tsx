...
export default function ListAllTransactions({}: ListAllTransactionsProps) {
    ...
    const [mutableSelectedDate, setMutableSelectedDate] = useState(selectedDate);
    const monthLong = mutableSelectedDate.toLocaleDateString('pt-BR', {month: 'long'});
    
    const handleDecreaseMonth = () => {
        const date = new Date(mutableSelectedDate);
        date.setMonth(date.getMonth() - 1);
        setMutableSelectedDate(date);
    }
    ...
    const {data, isLoading, isError} = useQuery<Types.DataAllTransactionsType>({
        queryKey: ["listalltransactions-data", mutableSelectedDate],
        queryFn: () => api.getAllTransactionsByMonth(mutableSelectedDate.toISOString())
    });
    console.log(data);
  ...
  return (
    <div className="min-h-dvh bg-surface flex flex-col text-body gap-6">
        ...
        <div className="flex gap-6 items-center justify-center">
            <IconButton variant={"ghost"} className="text-primary" onClick={handleDecreaseMonth}><ChevronLeft /></IconButton>
            <span className="label-large text-primary" onClick={() => setShowDatePicker(true)}>{monthLong.charAt(0).toUpperCase() + monthLong.slice(1)}</span>
            <IconButton variant={"ghost"} className="text-primary" onClick={handleIncreaseMonth}><ChevronRight /></IconButton>
        </div>
                    ...
                    if(arr.length - 1 === i){
                        ...
                        <div key={i} className="flex px-4 py-3.5 gap-4 justify-between items-center">
                            ...
                            <div className="flex flex-col items-end">
                                <span className={`label-medium ${d.type === 'gain' ? 'text-positive' : 'text-negative'}`}>{"R$" + currencyFormat(d.amount)}</span>
